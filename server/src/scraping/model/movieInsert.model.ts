...
class MovieDAO {
  ...
  // 장르 ID 가져오기
  static async getGenreId(genreNm: string) {
    try {
      const genre = await prisma.genre.findUnique({
        where: { name: genreNm },
        select: { genre_id: true },
      });
      return genre?.genre_id ?? null;
    } catch (err) {
      throw handlePrismaError(err);
    }
  }
  ...
  // 영화와 장르 관계 추가
  static async insertMovieAndGenre(movieId: number, genreId: number) {
    try {
      await prisma.movie_and_genre.create({
        data: {
          movie_id: movieId,
          genre_id: genreId,
        },
