...
import { CreateUserDTO, JwtUserDTO } from "dtos/user.dto";
...
export default {
  ...
  refreshToken: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // RefreshToken 검증
      const { id, provider } = await tokenUtil.verifyRefreshToken(refreshToken);

      // RefreshToken은 유효인 경우, AccessToken 생성
      let newAccessToken = await tokenUtil.generateAccessToken({ id, provider });
  ...
  login: (req: Request, res: Response, next: NextFunction) => {
    passport.authenticate("local", { session: false }, async (err: AppError | Error, user: JwtUserDTO) => {
